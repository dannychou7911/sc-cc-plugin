...
程式碼揭示 what，但 git 歷史常保存著 why。在問使用者之前，先向 git 歷史找答案——這能直接減少 Q&A 輪數，並提高 Why 記錄的品質。
...
**流程**：
1. `git blame -L {起},{迄} {檔案}` 定位目標行的 commit
2. `git show --no-patch {commit}` 讀取 commit message，抽取票號（如 footer 的 `P26-486`）
3. GitLab MCP 可用時，進一步以票號或 commit 查 MR 描述與討論串
4. 找到來源 → 該項目升級為 `[推斷]` 或 `[確認]`，Why 後附證據：`（來源：commit abc1234 / MR !123）`
5. 仍無解 → 維持 `[待確認]` 交 Q&A

**Phase 0 前置**：先以 `git log --oneline -30` 與 `git tag --list` 概覽歷史，辨識該 repo 的 commit 慣例（訊息格式、票號位置），後續考古才知道往哪裡找。

## i18n 檔案作為行為側寫

locale 檔（如 `locales/zh-TW.json`）通常枚舉了所有 UI 文案與錯誤訊息，是補全 L2「錯誤處理」表格的高效來源。

**三條規則**：
1. **單向性**：i18n 只是補充訊號，程式碼才是 source of truth。**不得**因 i18n 缺少某訊息就判定功能未做錯誤處理（i18n 可能過期、或訊息 hardcode 在程式碼中）
2. **錯誤 key 反查**：i18n 中的錯誤訊息 key → 反查程式碼中的引用位置 → 確認觸發條件後補入 L2 錯誤處理表格
3. **孤兒 key**：i18n 有、程式碼無任何引用的 key，是死碼或已棄用功能的訊號，標 `[待確認]` 交 Q&A

## 死碼與已棄用功能偵測

在 Q&A 詢問「已廢棄功能」之前，先靜態偵測候選項，把開放式問題轉為具體確認題（使用者好回答十倍）：

| 訊號 | 偵測方式 |
|------|---------|
| route 定義了但無導覽入口 | 比對路由表與導覽選單、頁面連結 |
| export 了但無人 import | LSP Find References 優先，退回 Grep 符號引用 |
| 註解暗示棄用 | Grep `deprecated`、`棄用`、`勿用`、被註解掉的大段程式碼 |
| i18n 孤兒 key | 見「i18n 檔案作為行為側寫」 |

偵測結果標 `[待確認]`，在業務脈絡收集時以具體問題確認（如「`/legacy-report` 無任何入口指向，是否已廢棄？」）。
