...
每份 L2 文件的最後一節為驗收場景，把行為描述轉為 QA 可直接使用的 Gherkin 場景——B 可測試性評分的分子即為場景數，寧可少而精確，不要湊數。

- 涵蓋：主成功路徑 + 錯誤路徑 + 邊界情況
- 場景基於**當前行為**：若當前行為疑似 bug（與預期行為不一致），場景必須標註，避免 QA 把 bug 當驗收標準
- 沿用 Confidence Marker（`[確認]` / `[推斷]`）
- 是否產出本節，依首份 L2 檢查點時使用者的選擇（記錄於進度檔工作偏好）
...
（依「通用規範 › 驗收場景」撰寫：主成功路徑 + 錯誤路徑 + 邊界情況。若使用者於檢查點選擇不產出，省略本節）
```
